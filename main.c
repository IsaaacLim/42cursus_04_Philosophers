...
void	philo_routine(t_philos *philo, char *color)
{
	...
	{
		pthread_mutex_lock(&fork_x[philo->fork_a]);
		ft_print(philo->x, "has taken a fork");
	//	ft_print(philo->x, "has taken a fork", color);
	//	printf("\t%d\n", philo->fork_a);
		pthread_mutex_lock(&fork_x[philo->fork_b]);
		ft_print(philo->x, "has taken a fork");
	//	ft_print(philo->x, "has taken a fork", color);
	//	printf("\t%d\n", philo->fork_b);
	//	usleep(50);
...
//		philo->t_last_meal = ft_time();
		pthread_mutex_unlock(&fork_x[philo->fork_a]);
		pthread_mutex_unlock(&fork_x[philo->fork_b]);
...
void	*ft_philo_checker(void *arg)
{
	...
	while (!g_argv.all_finished && !g_argv.dead)
	{
		...
		if (ft_time() - philo->t_last_meal > (g_argv.dying / 1000) + 8)
		{
			//usleep(6000);
		//	if (ft_time() - philo->t_last_meal > (g_argv.dying / 1000) + 5)
		//	{
				ft_print(philo->x, "died");
...
int	main(int argc, char *argv[])
{
	...
	while (n < g_argv.n_philos)
	{
		...
		if (n % 2 == 0)
		{
			philo[n]->fork_a = (n + 1) % g_argv.n_philos;
			philo[n]->fork_b = n;
		}
		else
		{
			philo[n]->fork_a = n;
			philo[n]->fork_b = (n + 1) % g_argv.n_philos;
		}
		philo[n]->n_eaten = 0;
